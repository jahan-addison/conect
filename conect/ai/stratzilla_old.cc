/*
    The code below is a modified version of the @stratzilla Alpha-beta
...
void printBoard(gui::Layout&);
int userMove();
void makeMove(gui::Layout&, int, unsigned int);
void errorMessage(int);
int aiMove();
gui::Layout copyBoard(gui::Layout);
bool winningMove(gui::Layout&, unsigned int);
int scoreSet(vector<unsigned int>, unsigned int);
int tabScore(gui::Layout, unsigned int);
array<int, 2> miniMax(gui::Layout&, unsigned int, int, int, unsigned int);
...
unsigned int MAX_DEPTH =
  5; // the default "difficulty" of the computer controlled AI
    ...
    while (!gameOver) {                  // while no game over state
        if (currentPlayer == COMPUTER) { // AI move
            ...
            makeMove(board, userMove(), PLAYER);
                   ...
                   NUM_ROW * NUM_COL) { // if max number of turns reached
            gameOver = true;
        }
 ...
 */
void
makeMove(gui::Layout& b, int c, unsigned int p)
{
...
userMove()
{
    ...
    while (true) { // repeat until proper input given
        ...
        } else if (!((unsigned int)move < NUM_COL &&
                     move >= 0)) {                  // if out of bounds
        ...
        for (unsigned int c = 0; c < NUM_COL; c++) { // for each possible move
            if (b[NUM_ROW - 1][c] == 0) { // but only if that column is non-full
                vector<vector<int>> newBoard =
                  copyBoard(b);           // make a copy of the board
 ...
 */
int
tabScore(vector<vector<int>> b, unsigned int p)
{
    int score = 0;
 ...
 */
int
scoreSet(vector<unsigned int> v, unsigned int p)
{
    ...
    for (unsigned int i = 0; i < v.size();
         i++) { // just enumerate how many of each
        good += (v[i] == p) ? 1 : 0;
 ...
 */
int
heurFunction(unsigned int g, unsigned int b, unsigned int z)
{
    int score = 0;
    if (g == 4) {
        score += 500001;
 ...
 */
bool
winningMove(vector<vector<int>>& b, unsigned int p)
{
 ...
 */
void
printBoard(vector<vector<int>>& b)
{
...
//         }
//         if (flag) {
//             cout << "Invalid command line argument, using default depth = 5."
//                  << endl;
//         } else {
//             MAX_DEPTH = i;
//         }
